...
describe("API methods", () => {
  ...
  test("POST/PUT/PATCH/DELETE endpoints send expected payload", async () => {
    ...
    await API.installDemoData({ profile_id: "showcase-core-v1", frontend_state: { lists: { targets: [] } } });
    expect(fetch).toHaveBeenLastCalledWith(
      "http://127.0.0.1:8000/api/maintenance/demo/install",
      expect.objectContaining({
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ profile_id: "showcase-core-v1", frontend_state: { lists: { targets: [] } } }),
      })
    );
